...
class User(db.Model):
    ...
    comments = db.relationship("Comment", backref="user")  # 评论 外键关联
    moviecols = db.relationship("Moviecol", backref="user")  # 电影收藏 外键关联
...
class Userlog(db.Model):
    ...
    addtime = db.Column(db.Date, index=True, default=datetime.now)  # 最近登录时间

    def __repr__(self):
        return "<Userlog {!r}>".format(self.id)
...
class Movie(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    comments = db.relationship("Comment", backref="movie")  # 评论 外键关联
...
class Admin(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    adminlogs = db.relationship("Adminlog", backref="admin")  # 管理员日志 外键关联
...
class Adminlog(db.Model):
    ...
    def __repr__(self):
        return "<Adminlog {!r}>".format(self.id)
...
class Oplog(db.Model):
    ...
    addtime = db.Column(db.Date, index=True, default=datetime.now)  # 最近操作时间

    def __repr__(self):
        return "<Oplog {!r}>".format(self.id)
...
if __name__ == "__main__":
    ...
    # admin = Admin(
    #     name="finger1",
    #     pwd=generate_password_hash("finger"),
    #     is_super=0,
    #     role_id=1
    # )
